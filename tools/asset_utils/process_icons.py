...
def remove_background_and_center(
    icon: Image.Image, threshold: int = 60, target_size: int = 512
) -> Image.Image:
    """
...
class SpriteSheetConfig:
    """Configuration for sprite sheet processing."""
...
def process_sprite_sheet(config: SpriteSheetConfig) -> None:
    print(f"Processing {config.filepath}...")

    img = Image.open(config.filepath).convert("RGBA")
    w, h = img.size
    cell_w, cell_h = w // config.cols, h // config.rows
    ...
    for row in range(config.rows):
        for col in range(config.cols):
            # Crop
            left = col * cell_w + config.inset
            upper = row * cell_h + config.inset
            right = (col + 1) * cell_w - config.inset
            lower = (row + 1) * cell_h - config.inset
            ...
            # Save
            filename = f"{config.name_prefix}_{count:02d}.png"
            final.save(os.path.join(config.output_dir, filename))
...
if __name__ == "__main__":
    ...
    process_sprite_sheet(
        SpriteSheetConfig(
            filepath="src/assets/ui_icons/layer_icons.png",
            output_dir="src/assets/ui_icons/layers/denoised512",
            name_prefix="layer",
            cols=4,
            rows=4,
            inset=0,
    ...
    process_sprite_sheet(
        SpriteSheetConfig(
            filepath="src/assets/ui_icons/sidebar_icons.png",
            output_dir="src/assets/ui_icons/sidebar/denoised512",
            name_prefix="sidebar",
            cols=4,
            rows=4,
            inset=0,
