...
from domain.entities.viewport_entity import ViewportEntity, OutputPreset
from domain.modules.layout_manager import create_coordinate_mapper, CoordinateMapper
...
# Local Utils
from domain.modules.pipeline_manager.scenario.visual.layout_test_utils import (
    setup_output_dir,
...
class TestLayoutViewport(unittest.TestCase):
    ...
    def test_run_layout_manager(self):
        ...
        for idx, layer in enumerate(layers_list):
            ...
            world_pos = layer.position  # Center position
            ...
            report_lines.append(f"  World Position (center): ({int(world_pos.x)}, {int(world_pos.y)})")
            report_lines.append(f"  World Bounds: x={int(world_bounds.x)}, y={int(world_bounds.y)}, "
                               f"w={int(world_bounds.width)}, h={int(world_bounds.height)}")
            report_lines.append(f"  Viewport Position: ({int(viewport_pos.x)}, {int(viewport_pos.y)})")
            report_lines.append(f"  Viewport Bounds: x={int(viewport_bounds.x):.0f}, y={int(viewport_bounds.y):.0f}, "
        ...
        for layer in sorted(layers_list, key=lambda l: l.z_index):
            ...
            try:
                ...
                print(f"    Rendered: {layer.name} at ({paste_x}, {paste_y}) size {scaled_w}x{scaled_h}")
        ...
        debug_draw.line([(center - 30, center), (center + 30, center)], fill=(255, 0, 0, 255), width=2)
        debug_draw.line([(center, center - 30), (center, center + 30)], fill=(255, 0, 0, 255), width=2)
