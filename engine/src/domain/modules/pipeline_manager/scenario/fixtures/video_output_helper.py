...
import cv2
import numpy as np
from PIL import Image  # noqa: PLR0913, PLR0912
...
def write_frame(self, pil_image, save_to_gif=True, gif_interval=2):
    ...
    if img_np.shape[2] == self.RGBA_CHANNELS:
    ...
    # Accumulate for GIF
    if save_to_gif and (len(self.gif_frames) % gif_interval == 0):
        # Ideally we need a frame counter, but let's just append everything the caller asks us to.
...
def finalize(self):
    ...
    if self.gif_frames:
        print("Saving GIF...")
        duration = 1000 / (self.fps / 2) # noqa: PLR0913, PLR0912
